...
for i in range(size): #X1, EBFP2
    for j in range(size): #X2, MKO2
        for k in range(size): #X3 MMAROON1
            #T:
            #Csy4 node: X1: Csy4 (10ng), X3: 1xCsy4R_CasE (100ng)
            n_A[i][j][k] = EF6('Csy4', ['ERN', 'REC'], [X1[i], X3[k]], [0.2, 2.0], ['0x', '1x'], 'ERN') #output is CasE

            #T:
            #CasE node: X2: 4xG (100ng), X1: 8xCasE (100ng), X3: CasER_mNG (150ng)
            n_C[i][j][k] = EF6('CasE', ['T-ERN', 'ERN', 'REC', 'T-REC-N'], [n_A[i][j][k], X1[i], X3[k], X2[j]], [1.0, 2.0, 3.0, 2.0], ['0x', '8x', '0x', '4x'], 'FP')
...
for i2 in range(size):
    for j2 in range(size):
        for k2 in range(size):
            ...
            if (X3[k2] > 95662) and (X3[k2] < 233688):
...
ax.set_title(title_T, fontsize = 28, pad=20)
