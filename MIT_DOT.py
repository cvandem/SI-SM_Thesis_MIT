...
for i in range(size): #X1, EBFP2
    for j in range(size): #X2, MKO2
        for k in range(size): #X3 MMAROON1
            #DOT:
            #Pgu node: Pgu 200ng (X1), Pgu 200ng (X2), 50ng 8xPguRCsy4 (X3)
            n_A[i][j][k] = EF6('Pgu', ['ERN', 'ERN', 'REC'], [X1[i], X2[j], X3[k]], [4.0, 4.0, 1.0], ['0x', '0x', '8x'], 'ERN') #output is Csy4

            #DOT:
            #CasE node: 50ng CasE (X2), 50ng 8xCasER_Csy4 (X1)
            n_B[i][j][k] = EF6('CasE', ['ERN', 'REC'], [X2[j], X1[i]], [1.0, 1.0], ['0x', '8x'], 'ERN') #output is Csy4

            #DOT:
            #Csy4 node: 50ng 1xCsy4 (X2), 200ng Csy4R_mNG (X3)
            n_C[i][j][k] = EF6('Csy4', ['T-ERN', 'T-ERN', 'ERN', 'REC'], [n_A[i][j][k], n_B[i][j][k], X2[j], X3[k]], [1.0, 1.0, 1.0, 4.0], ['0x', '0x', '1x', '0x'], 'FP') #output is mNG
...
for i2 in range(size):
    for j2 in range(size):
        for k2 in range(size):
            ...
            if (X3[k2] > 117771) and (X3[k2] < 307899):
...
ax.set_title(title_DOT, fontsize = 28, pad=20)
