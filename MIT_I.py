...
for i in range(size): #X1, EBFP2
    for j in range(size): #X2, MKO2
        for k in range(size): #X3 MMAROON1
            #I:
            #Csy4 node:
            n_A[i][j][k] = EF6('Csy4', ['ERN', 'REC'], [X1[i], X3[k]], [2.0, 2.0], ['8x', '0x'], 'ERN') #output is CasE

            #I:
            #Pgu node
            n_B[i][j][k] = EF6('Pgu', ['ERN', 'REC'], [X3[k], X1[i]], [2.0, 2.0], ['8x', '8x'], 'ERN') #output is CasE

            #I:
            #CasE node
            n_C[i][j][k] = EF6('CasE', ['T-ERN', 'T-ERN', 'REC'], [n_A[i][j][k], n_B[i][j][k], X3[k]], [1.0, 1.0, 2.0], ['0x', '0x', '0x'], 'FP') #output is mNG
...
for i2 in range(size):
    for j2 in range(size):
        for k2 in range(size):
            ...
            if (X3[k2] > 45898) and (X3[k2] < 111166):
...
ax.set_title(title_I, fontsize = 28, pad=20)
